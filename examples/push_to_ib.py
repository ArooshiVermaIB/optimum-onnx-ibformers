...
from ibformers.trainer.ib_utils import HF_TOKEN
...
def push_to_ib(model_path, pipeline_name, model_name):
    ...
    url = model.push_to_hub(model_name, commit_message='add model',
                            organization='instabase', private=True, use_auth_token=HF_TOKEN)

    # load tokenizer
    tokenizer: PreTrainedTokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=True)
    url_tokenizer = tokenizer.push_to_hub(model_name, commit_message='add tokenizer',
                                          organization='instabase', private=True, use_auth_token=HF_TOKEN)
