"""
https://github.com/alfonmedela/triplet-loss-pytorch/blob/master/loss_functions/triplet_loss.py
"""
...
def pairwise_distance_torch(embeddings, device):
    ...
    pairwise_distances_squared = torch.max(pairwise_distances_squared, torch.tensor([0.0]).to(device))
...
def TripletSemiHardLoss(y_true, y_pred, device, margin=1.0):
    ...
    masked_minimums = (
        torch.min(torch.mul(pdist_matrix_tile - axis_maximums[0], mask), dim=1, keepdim=True)[0] + axis_maximums[0]
    )
    ...
    masked_maximums = (
        torch.max(torch.mul(pdist_matrix - axis_minimums[0], adjacency_not), dim=1, keepdim=True)[0] + axis_minimums[0]
    )
