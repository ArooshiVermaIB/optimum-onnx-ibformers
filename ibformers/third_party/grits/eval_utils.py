...
def table_structure_to_cells(table_structures, table_spans, table_bbox):
    ...
    for column_num, column in enumerate(columns):
        for row_num, row in enumerate(rows):
            ...
            cell = {"bbox": list(cell_rect), "column_nums": [column_num], "row_nums": [row_num], "header": header}

            cell["subcell"] = False
            for supercell in supercells:
                supercell_rect = Rect(list(supercell["bbox"]))
                if (supercell_rect.intersect(cell_rect).getArea() / cell_rect.getArea()) > 0.5:
                    cell["subcell"] = True
                    break
    ...
    for supercell in supercells:
        ...
        for subcell in subcells:
            ...
            if (subcell_rect.intersect(supercell_rect).getArea() / subcell_rect_area) > 0.5:
